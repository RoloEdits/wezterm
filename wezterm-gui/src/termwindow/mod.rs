...
impl TermWindow {
    ...
    fn clear_all_overlays(&mut self) {
        let overlay_panes_to_cancel = self
            .pane_state
            .borrow().values().filter_map(|state| state.overlay.as_ref().map(|overlay| overlay.pane.pane_id()))
            .collect::<Vec<_>>();
...
impl TermWindow {
    ...
    fn update_title_impl(&mut self) {
        ...
        let title = match config::run_immediate_with_lua_config(|lua| {
            if let Some(lua) = lua {
                let tabs = lua.create_sequence_from(tabs.clone())?;
                let panes = lua.create_sequence_from(panes.clone())?;
    ...
    fn move_floating_pane_to_split(&self, split_direction: SplitDirection) {
        ...
        promise::spawn::spawn(async move {
            ...
            mux.move_floating_pane_to_split(pane.pane_id(), split_direction)
                .await?;
            Result::<(), anyhow::Error>::Ok(())
        })
        .detach();
